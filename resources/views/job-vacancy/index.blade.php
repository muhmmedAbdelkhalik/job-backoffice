            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($jobVacancies as $jobVacancy)
                        <tr
                            @if (!$jobVacancy->trashed()) class="cursor-pointer hover:bg-gray-100"  onclick="window.location.href='{{ route('job-vacancy.show', $jobVacancy->id) }}'" @endif>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jobVacancy->title }}</td>
                            @if ($isAdmin)
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    {{ $jobVacancy->company?->name ?? 'No Company' }}</td>
                            @endif
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jobVacancy->location }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jobVacancy->type }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ number_format($jobVacancy->salary, 0) }} USD</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $jobVacancy->created_at->format('d M Y') }}</td>
